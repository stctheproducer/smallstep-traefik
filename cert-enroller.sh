...
if [ ! -f $TLS_CERT_LOCATION ]; then
	echo "Running command: step ca certificate $tls_subject $TLS_CERT_LOCATION $TLS_KEY_LOCATION ${flags[@]}"
	step ca certificate $tls_subject $TLS_CERT_LOCATION $TLS_KEY_LOCATION ${flags[@]}
else
	echo "Not running step ca command"
fi
